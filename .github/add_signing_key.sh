...
security import DeveloperIDApplication.cer  -k  ~/Library/Keychains/$KEY_CHAIN -T /usr/bin/codesign
security import DeveloperIDInstaller.cer  -k  ~/Library/Keychains/$KEY_CHAIN -T /usr/bin/codesign
security import key.p12  -k  ~/Library/Keychains/$KEY_CHAIN -P $KEY_PASSWORD -T /usr/bin/codesign
security unlock-keychain -p travis $KEY_CHAIN
